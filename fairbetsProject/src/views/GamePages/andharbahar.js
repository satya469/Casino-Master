...
class Casino extends React.Component {
  ...
  componentDidMount() {
    this.props.data_load(providerconfig["ANDHARBAHAR"], "11", "AHDARBAHARSLIDERS",{TYPE: "Andar Bahar"})
    ...
    if (this.state.scrollevent) {
      ...
      if (scrolled >= 1) {
        ...
        if (this.state.data.length < index1) {
          ...
          this.props.get_scrollevent_load(this.props.setprovider[this.state.select], this.props.dataList.allData,{TYPE: "Andar Bahar"})
  ...
  typesActive = value => {
    this.props.gametypechange(value, providerconfig.ANDHARBAHAR)
  }

  provider_change = value => {
    this.props.providerchange(value, providerconfig.ANDHARBAHAR,{TYPE: "Andar Bahar"})
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        {
          indata && indata.length > 0 ? <div style={{ minHeight: "25rem" }}>
            <Row className="casino-item-group">
              {indata.map((item, i) => (
                <Col key={i} className="col-2-4 item" xs="4" sm="4" md="3" lg="2">
                  <CasinoItem data={item} me={this.props} user={user} />
                </Col>
              ))}
            </Row>
...
const get_game = (state) => {
  ...
  return {
    casinoSlider_images: state.sliders.andarbahar_images,
