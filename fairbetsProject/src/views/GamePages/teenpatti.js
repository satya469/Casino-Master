...
class Casino extends React.Component {
  ...
  componentDidMount() {
    this.props.data_load(providerconfig["TEENPATTI"], "13", "TEENPATTISLIDERS",{TYPE: "Teen Patti"})
    ...
    if (this.state.scrollevent) {
      ...
      if (scrolled >= 1) {
        ...
        if (this.state.data.length < index1) {
          ...
          this.props.get_scrollevent_load(this.props.setprovider[this.state.select], this.props.dataList.allData,{TYPE: "Teen Patti"})
  ...
  typesActive = value => {
    this.props.gametypechange(value, providerconfig.TEENPATTI)
  }

  provider_change = value => {
    this.props.providerchange(value, providerconfig.TEENPATTI,{TYPE: "Teen Patti"})
  }
              ...
              <Row className="casino-item-group">
                {indata.map((item, i) => (
                  <Col key={i} className="col-2-4 item" xs="4" sm="4" md="3" lg="2">
                    <CasinoItem user={user} data={item} me={this.props} />
                  </Col>
                ))}
              </Row>
...
const get_game = (state) => {
  ...
  return {
    casinoSlider_images: state.sliders.teenpatti_images,
