...
class Casino extends React.Component {
  ...
  componentDidMount() {
    this.props.data_load(providerconfig["HORSERACE"], "12", "HORSERACESLIDERS")
  ...
  typesActive = value => {
    this.props.gametypechange(value, providerconfig.HORSERACE)
  }

  provider_change = value => {
    this.props.providerchange(value, providerconfig.HORSERACE)
  }
              ...
              <Row className="casino-item-group">
                {indata.map((item, i) => (
                  <Col key={i} className="col-2-4 item" xs="4" sm="4" md="3" lg="2">
                    <CasinoItem data={item} user={user} me={this.props} />
                  </Col>
                ))}
              </Row>
...
const get_game = (state) => {
  ...
  return {
    casinoSlider_images: state.sliders.horserace_images,
