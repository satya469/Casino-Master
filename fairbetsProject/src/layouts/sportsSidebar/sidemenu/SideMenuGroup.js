...
import { Root,imageicon } from "../../../authServices/rootconfig"
          ...
          // const Tag = FaIcon[child.icon]
            return (
              ...
              imageicon ? <React.Fragment>
                 <img alt={child.image} style={{width:"30px", height:"30px"}} src={Root.imageurl + (child.image)} />
              </React.Fragment> : <React.Fragment>
                  {
                    child.icon === 'cockIcon' ?  <Icon icon={roosterIcon} style={{fontSize:"25px"}} /> : child.icon === "faHourglass" ? <Icon icon={horseIcon} /> : <FontAwesomeIcon color="#8f99a3" icon={Icons[child.icon]}/>
                  }
              </React.Fragment>                                            
            }
